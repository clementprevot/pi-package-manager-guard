...
- No network calls, no telemetry, no data collection. On purpose.
...
First publish only: the package name must exist on npm and be linked to this repo as a trusted publisher (npmjs.com, package settings, "Trusted Publisher": repository `clementprevot/pi-package-manager-guard`, workflow `release.yml`). Publish the first version locally with `npm publish` while logged in, then link the trusted publisher so later tags publish automatically.
